...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        /// <summary>
        /// Saves a waypoint writer file for ZEROTECH GEMINI
        /// </summary>
        private void savewaypointsZero()
        {
            SaveFileDialog fd = new SaveFileDialog();
            fd.Filter = "Ardupilot Mission (*.rw)|*.*";
            fd.DefaultExt = ".rw";
            fd.FileName = wpfilename;
            DialogResult result = fd.ShowDialog();
            string file = fd.FileName;
                    ...
                    for (int a = 0; a < Commands.Rows.Count - 0; a++)
                    {
                        ...
                        switch (mode)
                        {
                            case 178:  //do_change_speed
                                dat = Convert.ToDouble((Commands.Rows[a].Cells[Param2.Index].Value));
                                speed = dat * 255 / 10.2;
                                sw.Write(double.Parse(Commands.Rows[a].Cells[Lon.Index].Value.ToString()).ToString("0.00000"));
                                sw.Write(" " + double.Parse(Commands.Rows[a].Cells[Lat.Index].Value.ToString()).ToString("0.00000"));
                                temp = Convert.ToInt32((Commands.Rows[a].Cells[Alt.Index].Value));
                                if (temp == 0)
                                {
                                    sw.Write(" " + "9999");
                                }
                                ...
                                {
                                    sw.Write(" " + (double.Parse(Commands.Rows[a].Cells[Alt.Index].Value.ToString()) / CurrentState.multiplierdist).ToString("0", new System.Globalization.CultureInfo("en-US")));
                                }
                                sw.Write(" " + "0");
                                sw.Write(" " + speed);
                                sw.Write(" " + "65535");
                            ...
                            case 206:  //do_set_cam_trig_dist
                                ...
                                sw.WriteLine("");
                                break;
                            default:  //default / keep_previous_state
                                sw.Write(double.Parse(Commands.Rows[a].Cells[Lon.Index].Value.ToString()).ToString("0.00000"));
                                sw.Write(" " + double.Parse(Commands.Rows[a].Cells[Lat.Index].Value.ToString()).ToString("0.00000"));
                                temp = Convert.ToInt32((Commands.Rows[a].Cells[Alt.Index].Value));
                                if (temp == 0)
        ...
        private void SaveZeroFile_Click(object sender, EventArgs e)
        {
            savewaypointsZero();
            writeKML();
        }
        ...
        private void BUT_loadwpfile_Click(object sender, EventArgs e)
        {
            ...
            fd.Filter = "All Supported Types|*.txt;*.shp;*.rw|Ardupilot Mission (*.txt)|*.*|Shape file|*.shp";
            DialogResult result = fd.ShowDialog();
            string file = fd.FileName;
        ...
        */
        ...
        private void saveZEROTECHWPStripMenuItem2_Click(object sender, EventArgs e)
        {
            SaveZeroFile_Click(null, null);
        }
